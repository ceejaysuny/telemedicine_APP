...
router.get('/profile', async (req, res) => {
    ...
    try {
        const [patient] = await db.execute('SELECT first_name, last_name, phone, email, date_of_birth, gender, address FROM Patients WHERE id = ?', [req.session.user.id]);
...
router.post('/appointments', authMiddleware, async (req, res) => {
    ...
    try {
        await db.execute('INSERT INTO Appointments (patient_id, doctor_id, appointment_date, appointment_time, reason, status) VALUES (?, ?, ?, ?, ?, ?)', 
            [req.session.user.id, doctor_id, appointment_date, appointment_time, reason, 'scheduled']);
...
router.get('/specific_appointments/:id', authMiddleware, async (req, res) => {
    const appointmentId = req.params.id;
    try {
        const [appointment] = await db.execute('SELECT * FROM Appointments WHERE id = ?', [appointmentId]);
...
router.get('/appointments', authMiddleware, async (req, res) => {
    ...
    if (req.session.user.role === 'admin') {
        ...
        query = 'SELECT * FROM Appointments WHERE patient_id = ? AND status != ?';
        params = [req.session.user.id, 'canceled'];
...
router.put('/appointments/:id', authMiddleware, async (req, res) => {
    ...
    try {
        const [result] = await db.execute(
            ...
            [appointment_date, appointment_time, reason, status, appointmentId]
        );
...
router.put('/appointments/:id', authMiddleware, async (req, res) => {
    ...
    try {
        await db.execute('UPDATE Appointments SET appointment_date = ?, appointment_time = ?, reason = ?, status = ? WHERE id = ?', 
            [appointment_date, appointment_time, reason, status, appointmentId]);
        res.json({ message: 'Appointment updated successfully' });
    } catch (err) {
        res.status(500).json({ error: err.message });
...
// Cancel appointment
router.put('/appointments/cancel/:id/', authMiddleware, async (req, res) => {
    const appointmentId = req.params.id;
...
router.get('/doctors', authMiddleware, async (req, res) => {
    try {
        const [doctors] = await db.execute('SELECT id, first_name, last_name, specialization, schedule FROM Doctors');
        res.json(doctors);  // Make sure you're returning JSON here
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
